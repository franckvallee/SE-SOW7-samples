 ...
 */
/* global Batch, ESGW, ESEvent, GRAPHQL, GQLGW, java, IO */
var OUTPUT_DIR = "/shared/exports/ProductSynchro";
...
while (batchIterator.hasNext()) {
    ...
    var eventDataPool = ESEvent.getEventData( ESGW, nodeID, typeID);
    var gqlToken;
    if ( eventDataPool.length > 0 ) {
        gqlToken = GRAPHQL.getBearerToken( GQLGW);
    }
    for each ( var eventData in eventDataPool ) {
        var contextID = eventData.context;
        ...
        productContent.event = eventData.data;
        ...
        ESEvent.deleteByID( ESGW, contextID + "|" + typeID + "|" + nodeID);
    }
}
